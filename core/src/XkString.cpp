...
String XkPublicFunction to_s(float* arr, size_t length)
{
 std::ostringstream s;
...
String XkPublicFunction to_s(unsigned int* arr, size_t length)
{
 std::ostringstream s;
 for (size_t i=0;i < length;i++)
  s << arr[i] << " ";
...
{
 return hash(Xk::String(str));
}

StringHash hash(const Xk::String& str)
{
 static HashFunction<Xk::String> function;
 return function(str);
}
...
bool split(const String& str, Xk::map<String, String>& value, char delimiter, bool lowerKey, bool trim)
{
 size_t pos = str.find_first_of(delimiter);
 if (pos == String::npos)
  return false;
...
/*
void getLine(Xk::Resource* resource, Xk::buffer<char>& buffer)
{
 buffer.remove_all();
 while(1)
 {
