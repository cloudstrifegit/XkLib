...
template<typename T> class buffer
{
 ...
 public:
  ...
  {
   if (mFirst)
    GC::safe_free<T, FrequentOperationsAllocator>(mFirst, XK_GC_THIS);
  }
  ...
  */
  inline void resize(size_t new_size)
  {
   ...
   Xk_DebugPrint_Buffer("resize type='" << typeid(T).name() << "', first => '" << mFirst << "', current_size = '" << size() << "', new_size = '" << new_size << "'");
